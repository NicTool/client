...
function onLoggedIn (response) {
    ...
    document.getElementById('groups_ul').innerHTML = `<li gid="${response.group.id}">${response.group.name}</li>`;
...
function showZones () {
    ...
    .then((response) => {
        ...
        for (const z of response.zone) {
            ...
            document.getElementById(`zone_${z.id}_tr`).addEventListener('click', (event) => {
                showZoneRecords(z);
            })
        }
    })
...
function showZoneRecords (zone) {
    ...
    .then((response) => {
        ...
        for (const zr of response.zone_record) {
            ...
            const row = document.createElement('tr');
            try {
                const owner = zr.owner === `${zone.zone}.` ? zr.owner
                    : zr.owner.endsWith(`${zone.zone}.`) ? zr.owner
                    : `${zr.owner}.${zone.zone}.`;
                // console.log('owner', owner);
                const asRR = new RR[zr.type]({ ...zr, owner: owner });
                // console.log('asRR', asRR)
                row.asRR = asRR
                zr.rdata = asRR.getRdataFields().map(f => asRR.get(f)).join(' ')
            }
            catch (error) {
            ...
            document.getElementById(`zr_${zr.id}_tr`).addEventListener('click', (event) => {
...
function changeRDataField (name, rr, event) {
    let value = event.target.type === 'number' ? parseInt(event.target.value, 10)
        : /^\d+$/.test(event.target.value) ? parseInt(event.target.value, 10)
        : event.target.value
...
function populateZrEditRdata (rr, zr) {
    ...
    for (const f of rr.getRdataFields()) {
        ...
        t.addEventListener('change', (event) => { changeRDataField(f, rr, event) })
        t.addEventListener('keyup', (event) => { changeRDataField(f, rr, event) })
    }
}
...
function editZoneRecord (zone, zr, rr) {
    ...
    owner.addEventListener('change', (event) => { changeRDataField('owner', rr, event) })
    owner.addEventListener('keyup', (event) => { changeRDataField('owner', rr, event) })
    ...
    ttl.addEventListener('change', (event) => { changeRDataField('ttl', rr, event) })
    ttl.addEventListener('keyup', (event) => { changeRDataField('ttl', rr, event) })
    ttl.value = zr.ttl

    const type = document.getElementById('zrEditType')
    type.classList.remove('is-valid')
    type.classList.remove('is-invalid')    
    type.addEventListener('change', (event) => { changeRDataField('type', rr, event) })
    type.addEventListener('keyup', (event) => { changeRDataField('type', rr, event) })
    const typeRFCs = document.getElementById('zrEditTypeRFCs')
    typeRFCs.innerHTML = `RFCs: ${rr.getRFCs()
        .map(r => `<a href="https://tools.ietf.org/html/rfc${r}" target="_blank">${r}</a>`).join(', ')}`
    type.value = zr.type
    ...
    type.addEventListener('change', (event) => {
        ...
        typeRFCs.innerHTML = `RFCs: ${newRR.getRFCs()
        .map(r => `<a href="https://tools.ietf.org/html/rfc${r}" target="_blank">${r}</a>`).join(', ')}`
    })
...
document.getElementById('login_form_submit').addEventListener('click', (event) => {
    attemptLogin()
})
